...
double check_freq[33] = {25000000, 50000000, 75000000, 100000000, 150000000, 200000000, 300000000, 400000000, 500000000, 600000000, 700000000, 800000000, 900000000, 1000000000, 1100000000, 1200000000, 1300000000, 1400000000, 1500000000, 1600000000, 1700000000, 1800000000, 1900000000, 2000000000, 2100000000, 2200000000, 2300000000, 2400000000, 2500000000, 2600000000, 2700000000, 2800000000, 2900000000};
...
int find_power_level(uint64_t target_freq, float taregt_power)
{
  ...
  int closest_index = 0;
  uint32_t min_diff = fabsf(target_freq - check_freq[0]);
  for(int i = 1; i < 33; i++)
  {
    uint32_t current_freq = check_freq[i];
    uint32_t diff = fabsf(target_freq - current_freq) ;
    if(fabsf(diff) <= fabsf(min_diff))                //не знаю опчему, но модулей только верхних не хватает 
    {
      min_diff = diff;
      closest_index = i;
  ...
  //else
  //{
    float best_diff = fabsf(taregt_power - pgm_read_float(&power_table[0][closest_index]));                
    for(int level = 1; level <= power_level; level++ )
    {
      float current_dbm = pgm_read_float(&power_table[level][closest_index]);
      float diff = fabsf(taregt_power - current_dbm);
